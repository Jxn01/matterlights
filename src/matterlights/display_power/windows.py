"""Windows display-power monitoring via GUID_CONSOLE_DISPLAY_STATE.
...
Lifted verbatim from the flat ``display_power.py`` when the Linux port split
each platform-coupled subsystem into a subpackage. The only change is that the
class is no longer private, because the factory now imports it across a module
boundary.
"""
